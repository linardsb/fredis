...
- **Token metric (corrected in Phase 3, 2026-06-12):** `input_tokens + cache_read_input_tokens + cache_creation_input_tokens` from the **last main-thread `AssistantMessage`'s per-call usage** — a single API call's attention surface. Phase A originally read these fields from `ResultMessage.usage`, which sums them across every internal call of the turn and so multiply-counts cache reads — production rows showed 1.37M "tokens" on a 6-message thread (7-8× the 200k window) and a turn-1 double nudge at 241k. `input_tokens` alone is no fix either: with prompt caching it's only the uncached tail (~10 tokens/turn on a resumed session). Subagent calls are excluded via `parent_tool_use_id`.
...
- **Min-tier-gap guard (Phase 3):** hard is suppressed until the soft fire is at least `NUDGE_MIN_TURNS_BETWEEN_TIERS` (3) turns old, so soft gets a real action window before the escalation. The soft-fire turn is persisted in `chat_sessions.nudged_soft_turn_count` (`nudged_hard_turn_count` mirrors it for the hard tier); legacy rows with NULL turn counts keep pre-guard behaviour.
- **Where it's wired:** `engine.py:compute_thread_nudge` (pure decision function, fully unit-tested in `tests/test_chat_thread_nudge.py`); persisted via `Session.last_turn_context_tokens` / `nudged_soft_at` / `nudged_hard_at` / `nudged_soft_turn_count` / `nudged_hard_turn_count` (`session.py`).
- **Per-turn observability:** `[thread-nudge] tier=<soft|hard> turns=N tokens=N` printed to stdout the turn a nudge fires. Grep chat logs to verify firing is happening (or not, when you'd expect it). Post-Phase-3, `tokens=` must read ≤ the 200k window.
- **Tuning the thresholds:** edit `NUDGE_SOFT_TURNS / NUDGE_SOFT_TOKENS / NUDGE_HARD_TURNS / NUDGE_HARD_TOKENS / NUDGE_MIN_TURNS_BETWEEN_TIERS` constants at the top of `engine.py`. Restart the chat engine to pick up the change. Phase 4 (`/consolidate` directive) is gated on observing the corrected metric in production for 2+ days before any retune — do not retune in the same window as the metric fix.
