...
- **Heartbeat guardrail fail-closed three times: 2026-04-24 05:54 + 10:05, 2026-04-25 16:06.** First two were pre-Sonnet-promotion (commit `5069d4e`); the 16:06 timeout on 2026-04-25 was the first **post-promotion** recurrence, so the Haiku→Sonnet move did not fully resolve it. Next action: raise `asyncio.wait_for` from 15s to 30s in `heartbeat.py` and/or add single-retry before failing closed. `[impact: low, status: pending]`
