...
- **Heartbeat guardrail fail-closed twice on 2026-04-24** (05:54 + 10:05). Pre-promotion-to-Sonnet recurrence — both happened before commit `5069d4e` shipped. The Haiku-on-heartbeat fix should resolve it; if any further timeout occurs after 2026-04-25, raise `asyncio.wait_for` from 15s to 30s in `heartbeat.py` or add single-retry before failing closed. `[impact: low, status: monitoring]`
