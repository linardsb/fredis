...
- **Guardrail false-positive on `</external_data>` closing tag — now blocking reflection AND synthesis (2026-04-28, escalated 2026-04-30, re-escalated 2026-05-03).** Every non-timeout heartbeat on 2026-04-28 (7/7), 2026-04-30 (8/8), and 2026-05-02 (9/9) flagged the legitimate structural closing tag of the data wrapper as `xml_escape_attempt`. The 2026-04-30 and 2026-05-02 reflection passes both aborted (`reflection-aborted` source) — losing two days of MEMORY.md consolidation. **Further escalation 2026-05-03:** weekly synthesis pass also aborted (`synthesis-aborted` source) at 08:00 because the same pattern was detected in the daily-log bundle — now blocking the Sunday Dream-loop too. Two of the four memory loops (Consolidate + Dream) are now silently skipping due to a single false-positive pattern. Pattern matcher needs to either skip the wrapper's own structural tags or only fire when the tag appears *inside* an inner content block. `[impact: high, status: pending]`
